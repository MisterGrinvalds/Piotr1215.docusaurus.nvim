...
-- ========================================
-- Breadcrumb File Functions
-- ========================================

-- Write breadcrumb file to cwd after repo selection
local function write_docusaurus_breadcrumb()
	if not state.active_repo then
		return
	end

	local breadcrumb_path = vim.fn.getcwd() .. "/.docusaurus-nvim"
	local content = string.format(
		[[# Docusaurus Plugin Configuration
# This file was auto-generated by docusaurus.nvim

repo_name: %s
repo_path: %s
docusaurus_root: %s

# To change the active repo:
#   :Docusaurus repo_select
]],
		state.active_repo.name,
		state.active_repo.path,
		state.active_repo.docusaurus_root or "."
	)

	local file = io.open(breadcrumb_path, "w")
	if file then
		file:write(content)
		file:close()
	end
...
-- Read breadcrumb file from cwd to restore repo selection
local function read_docusaurus_breadcrumb()
	local breadcrumb_path = vim.fn.getcwd() .. "/.docusaurus-nvim"
	if vim.fn.filereadable(breadcrumb_path) ~= 1 then
		return nil
	end

	local lines = vim.fn.readfile(breadcrumb_path)
	local repo_name, repo_path, docusaurus_root
	...
	for _, line in ipairs(lines) do
		...
		if key == "docusaurus_root" then
			docusaurus_root = value
		end
	end

	if repo_name and repo_path then
		return {
			name = repo_name,
			path = repo_path,
			docusaurus_root = docusaurus_root or ".",
		}
	end
	return nil
end

-- Helper to get docusaurus directory from a repo object
local function get_docusaurus_dir_from_repo(repo)
	local repo_path = repo.path
	local docusaurus_root = repo.docusaurus_root or "."
...
-- Ensure a repo is selected, triggering selection if needed
-- Calls callback(repo) when repo is available
local function ensure_repo_selected(callback)
	-- Already selected?
	if state.active_repo then
		callback(state.active_repo)
		return
	end

	-- Try to restore from breadcrumb
	local restored = read_docusaurus_breadcrumb()
	if restored and vim.fn.isdirectory(restored.path) == 1 then
		state.active_repo = restored
		print("Restored active repo: " .. restored.name)
		callback(state.active_repo)
		return
	end

	-- No breadcrumb or invalid - force selection
	local registry = load_repos_registry()
	if vim.tbl_isempty(registry.repos) then
		print("No repos registered. Use :Docusaurus repo_import first.")
		return
	end

	-- Trigger repo selection with callback
	M.select_repo(callback)
end
...
function M.import_repo()
	...
	-- Validate docusaurus.config.js/ts exists
	local doc_dir = docusaurus_root == "." and repo_path or (repo_path .. "/" .. docusaurus_root)
	local config_js = doc_dir .. "/docusaurus.config.js"
	local config_ts = doc_dir .. "/docusaurus.config.ts"
	if vim.fn.filereadable(config_js) ~= 1 and vim.fn.filereadable(config_ts) ~= 1 then
		print(string.format("Warning: No docusaurus.config.js/ts found in: %s", doc_dir))
		print("This may not be a valid Docusaurus project. Build commands will fail.")
		local choice = vim.fn.confirm("Continue anyway?", "&Yes\n&No", 2)
		if choice ~= 1 then
			return
		end
	end
	...
	-- Write breadcrumb file for auto-restore
	write_docusaurus_breadcrumb()

	print(string.format("Active repo set to: %s", repo_name))
end

-- Select an external repo using Telescope picker
-- Optional callback is called with the selected repo after selection completes
function M.select_repo(callback)
	local registry = load_repos_registry()
			...
			attach_mappings = function(prompt_bufnr)
				actions.select_default:replace(function()
					...
					if vim.fn.isdirectory(repo_path) ~= 1 then
						print(string.format("Repository directory not found: %s", repo_path))
						local choice = vim.fn.confirm("Re-clone repository?", "&Yes\n&No", 2)
						if choice == 1 then
							print(string.format("Cloning %s...", repo.git_url))
							local base_dir = get_xdg_data_dir()
							if vim.fn.isdirectory(base_dir) ~= 1 then
								vim.fn.mkdir(base_dir, "p")
							end
							local clone_cmd = string.format("git clone '%s' '%s' 2>&1", repo.git_url, repo_path)
							local output = vim.fn.system(clone_cmd)
							if vim.v.shell_error ~= 0 then
								print("Failed to clone: " .. output)
								return
							end
					...
					local git_check = string.format("cd '%s' && git rev-parse --git-dir 2>&1", repo_path)
					vim.fn.system(git_check)
					if vim.v.shell_error ~= 0 then
						print(string.format("Not a git repository: %s", repo_path))
						return
					end
					...
					-- Call callback if provided (for ensure_repo_selected flow)
					if callback then
						callback(state.active_repo)
					end
				end)
...
-- Helper to get the docusaurus working directory (legacy, for sync usage)
...
function M.start_dev_server()
	ensure_repo_selected(function(repo)
		local doc_dir, err = get_docusaurus_dir_from_repo(repo)
		if not doc_dir then
			print(err)
			return
		end
...
M.write_docusaurus_breadcrumb = write_docusaurus_breadcrumb
M.read_docusaurus_breadcrumb = read_docusaurus_breadcrumb
M.ensure_repo_selected = ensure_repo_selected
